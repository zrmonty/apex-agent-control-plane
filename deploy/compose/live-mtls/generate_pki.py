...
try:
    ...
    _HOST_SECRETS_PATH = Path(__file__).with_name("host_secrets.py")
    _HOST_SECRETS_SPEC = importlib.util.spec_from_file_location(
        "apex_live_mtls_host_secrets", _HOST_SECRETS_PATH
    )
    if _HOST_SECRETS_SPEC is None or _HOST_SECRETS_SPEC.loader is None:
        raise RuntimeError(f"cannot load {_HOST_SECRETS_PATH}")
    _HOST_SECRETS_MODULE = importlib.util.module_from_spec(_HOST_SECRETS_SPEC)
    _HOST_SECRETS_SPEC.loader.exec_module(_HOST_SECRETS_MODULE)
    write_host_secrets = _HOST_SECRETS_MODULE.write_host_secrets
...
def _prepare_write(path: Path) -> None:
    if path.is_symlink():
        raise RuntimeError(f"refusing to write through symbolic-link destination: {path}")
    if path.exists():
        try:
            path.chmod(0o600)
        except OSError:
...
def main() -> None:
    ...
    out: Path = args.out
    if out.is_symlink():
        raise RuntimeError(f"refusing to write through symbolic-link output: {out}")
    if out.exists() and not out.is_dir():
        raise RuntimeError(f"refusing to write through non-directory output: {out}")
    ...
    nats_username = out / "nats-username"
    _prepare_write(nats_username)
    nats_username.write_text("ingest-publisher\n", encoding="utf-8")
    try:
        # Docker-readable; host tree gets 0600 via write_host_secrets.
        nats_username.chmod(0o644)
    except OSError:
        pass
    _write_runtime_secret(out / "nats-password")
    # The control gateway's own NATS account, never the ingest publisher's.
    control_nats_username = out / "control-nats-username"
    _prepare_write(control_nats_username)
    control_nats_username.write_text("control-publisher\n", encoding="utf-8")
    try:
        # Docker-readable; host tree gets 0600 via write_host_secrets.
        control_nats_username.chmod(0o644)
    except OSError:
        pass
    ...
    control_agent_tokens = out / "control-agent-tokens"
    control_agent_tokens.write_text(
        ";\n".join(
    ...
    try:
        control_agent_tokens.chmod(0o600)
    except OSError:
        pass
