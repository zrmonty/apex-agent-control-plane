...
// CI owns the whole ancestor chain; shared runner tool directories need not be protected.
const COSIGN: &str = "/apex-signature-tools/cosign";
...
fn dedicated_agent_uid_can_open_its_protected_cache() {
    ...
    SignatureVerifier::open(Path::new(COSIGN), Path::new("/signature-user-cache"))
...
fn live_cosign_accepts_exact_signer_and_rejects_wrong_identity() {
    let verifier = SignatureVerifier::open(Path::new(COSIGN), Path::new("/cosign-test-cache"))
