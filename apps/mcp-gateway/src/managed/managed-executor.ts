...
type ManagedExecutorOptions = Readonly<{
  ...
  filterOutput: (output: unknown, tool: ExposedTool) => FilteredOutput;
...
export class ManagedExecutor {
  ...
  async execute(toolAlias: string, input: unknown, headers: HeaderValues): Promise<unknown> {
    ...
    const request = this.authorizationRequest(tool, identity.subject);
    ...
    try {
      ...
      const rawOutput = await this.options.session.call(tool, parsedInput);
      const filtered = this.filter(rawOutput, tool);
      try {
        await this.options.emitEvidence({
          ...
          outputBytes: jsonSize(filtered.output),
  ...
  private findTool(alias: string): ExposedTool {
    const tool = this.options.config.exposedTools.find((candidate) => candidate.alias === alias);
    if (tool === undefined) {
  ...
  private authorizationRequest(tool: ExposedTool, subject: string): ManagedAuthorizationRequest {
    return {
  ...
  private filter(output: unknown, tool: ExposedTool): FilteredOutput {
    try {
      const filtered = this.options.filterOutput(output, tool);
      if (!Number.isSafeInteger(filtered.sourceBytes) || !Number.isSafeInteger(filtered.filteredBytes) || filtered.sourceBytes < filtered.filteredBytes) {
