...
use crate::proxy::{
    ...
    ProxyId, ProxyRevisionId, ProxySpec, ProxyToolClassification, ProxyTransport,
    RuntimeProfile, SecretRef, UpstreamBinding,
};
...
fn in_memory_store_contract() {
    exercise_store_contract(InMemoryProxyStore::default());
}

#[cfg(feature = "postgres")]
#[test]
fn postgres_store_contract() {
    let Some(url) = std::env::var("APEX_CONTROL_POSTGRES_URL").ok().filter(|value| !value.is_empty()) else {
...
fn exercise_store_contract<S>(store: S)
where
    S: ProxyStore + ProxyRevisionStore,
{
    ...
    let read_back = store.get(scope.clone(), proxy_id(PROXY_ID)).expect("get proxy");
    ...
    let updated = store
        ...
        .expect("store draft");
    let draft_revision_id = updated.draft_revision_id.clone().expect("draft revision id");
    ...
    let next_draft = store
        ...
        .expect("next draft");
    let frozen = store
        .get_revision(scope.clone(), proxy_id(PROXY_ID), published_revision_id.clone())
        .expect("frozen revision");
    assert_eq!(frozen.spec, published_spec);
    assert_eq!(frozen.revision_id, published_revision_id);
    assert_ne!(next_draft.draft_revision_id, Some(published.revision_id.clone()));

    let hidden = store.get(other_scope.clone(), proxy_id(PROXY_ID)).unwrap_err();
    assert_eq!(hidden.code(), "PROXY_NOT_FOUND");

    store
        .create(create_proxy(SECOND_PROXY_ID, "018f3d4a-8b9c-7d0e-8f12-3a4b5c6d7e8f", "alpha-proxy"))
        .expect("second proxy");
    store
        .create(create_proxy(THIRD_PROXY_ID, "018f3d4a-8b9c-7d0e-8f12-3a4b5c6d7e90", "beta-proxy"))
        .expect("third proxy");
    store
        .create(CreateProxy {
    ...
    let retired = store
        .retire(RetireProxy {
            ...
            scope: scope.clone(),
            proxy_id: proxy_id(PROXY_ID),
            expected_revision_id: Some(published.revision_id),
        })
        .expect("retire");
    assert_eq!(retired.lifecycle_state, ProxyLifecycleState::Retired);

    let tombstone = store
        .create(create_proxy("018f3d4a-8b9c-7d0e-8f12-3a4b5c6d7e92", "018f3d4a-8b9c-7d0e-8f12-3a4b5c6d7e93", "research-mcp-proxy"))
        .unwrap_err();
...
fn valid_proxy_spec() -> ProxySpec {
    ProxySpec {
        ...
        upstreams: vec![UpstreamBinding {
            ...
            credential_ref: Some(SecretRef::new("secret://vault/upstreams/portfolio-reader").expect("secret ref")),
            secret_refs: vec![],
        ...
        cli_profiles: vec![CliProfile {
            ...
            secret_refs: vec![SecretRef::new("secret://vault/cli/portfolio-token").expect("secret ref")],
        ...
        runtime_profile: RuntimeProfile {
            image_digest: "sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef".to_owned(),
