...
#[test]
fn startup_rejects_missing_staging_opt_in_before_network_access() {
    let output = run_with_env(&[]);
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("APEX_ALLOW_IN_MEMORY_IDEMPOTENCY=true"));
}

#[test]
fn startup_reports_missing_trusted_base_after_staging_opt_in() {
    let output = run_with_env(&[("APEX_ALLOW_IN_MEMORY_IDEMPOTENCY", "true")]);
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn startup_rejects_invalid_retry_budget_before_network_access() {
    let output = run_with_env(&[
        ("APEX_ALLOW_IN_MEMORY_IDEMPOTENCY", "true"),
