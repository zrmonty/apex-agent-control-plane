...
The [current-operation callback](operations/mcp-runtime-authority.md) now has a real mTLS/PostgreSQL server, fixed-file policy/enrollment refresh, bounded worker ownership and optional production-root registration. Its runtime-agent client checks actual Controller TLS and all snapshot bindings through a two-hop cross-process test. [Provisioning](operations/mcp-runtime-provisioning.md) has started with strict deployment-owned image catalog selection; production agent ingress, actual signature verification, secret staging and engine effects remain open, followed by routing and lifecycle work. This is still a read-only Task 7 checkpoint. Integer-microsecond callback fields do not close the end-to-end tracing requirement.
