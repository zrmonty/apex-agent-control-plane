...
The [current-operation callback](operations/mcp-runtime-authority.md) has a real mTLS/PostgreSQL server, fixed-file policy/enrollment refresh, bounded worker ownership and optional production-root registration. [Online deployment resolution](operations/mcp-runtime-deployment.md) now compiles the runtime configuration exclusively from the verified published revision and a protected, versioned deployment catalog. Its agent client validates the result through the same pinned channel; actual production-root/PG cross-process tests cover success and refusal. [Provisioning](operations/mcp-runtime-provisioning.md) includes image catalog selection, Linux Cosign verification and confined secret staging. Production agent ingress, launch-context/material binding, bounded effect-owner composition and durable container effects remain open, followed by routing and lifecycle work. These boundaries are not wired to EnsureRuntime; Task 7 and production readiness remain open. Integer-microsecond wire fields do not close the end-to-end tracing requirement.
