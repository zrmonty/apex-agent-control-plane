...
The server-side [current-operation callback](operations/mcp-runtime-authority.md) now has a real mTLS/PostgreSQL implementation, fixed-file policy/enrollment refresh, bounded worker ownership and optional production-root registration. This remains a read-only Task 7 checkpoint; the agent callback client, restricted provisioning/staging, routing, runtime lifecycle and remaining roadmap tasks are still required. Integer-microsecond callback fields do not close the end-to-end tracing requirement.
