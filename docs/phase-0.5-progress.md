...
| Multi-writer durable outbox and inbox | `startup::service::open_outbox` and `open_inbox` select separate Postgres-backed stores under `--features postgres` and `APEX_CONTROL_POSTGRES_URL`, with the file stores otherwise. See "Outbox backend selection" and "Durable delivery state" below. |
...
**The Postgres profile now shares both durability dimensions.** `APEX_CONTROL_POSTGRES_URL` selects a Postgres-backed event outbox and a separate `apex_control_inbox` table for delivery state. The inbox claims rows with `FOR UPDATE SKIP LOCKED`, records attempts in the same durable store, and preserves the file-backed implementation for single-process deployments. An agent polling replica B therefore sees a command accepted by replica A, while fanout completion remains independent from agent delivery. The live Postgres gate covers the shared submission path; the shared inbox uses the same verified TLS connection and schema-creation lock.

**Settled delivery state is bounded.** `APEX_CONTROL_COMMAND_RETENTION_SECS` defaults to 30 days and is validated to 1 hour through 365 days. A command that has reached the delivery-attempt ceiling is retained as an idempotency tombstone for that window, so a retry cannot create a second delivery; after expiry its identity may be reused. The file inbox compacts its journal into the active commands, latest delivery state, and live tombstones. The Postgres inbox's 60-second maintenance sweep deletes only settled rows outside the window, never undelivered or still-retryable commands. The event outbox uses the same retention window for completed idempotency rows, including file-journal compaction and an indexed Postgres delete, so the audit and delivery sides do not disagree about command reuse. A five-second reconciliation worker repairs both a crash between the outbox commit and inbox write and the less obvious case where fanout completed before the inbox write failed, by scanning recent completed payloads as well as pending rows. The sweep is independent of JetStream, so a broker outage cannot make delivery state grow without bound.
...
- **Tick interval: 5s** (`APEX_CONTROL_FANOUT_INTERVAL_SECS`, 1..=3600), matching `event-ingest`'s own outbox replay worker. A full batch drains immediately, so the configured tick is idle latency rather than a per-batch backlog penalty. Postgres deployments use a bounded pool (`APEX_CONTROL_POSTGRES_POOL_SIZE`, default 4, 1..=16) of independent synchronous clients; the file backend remains intentionally single-writer. Per-event retry deadlines are persisted by the file/Postgres backends, with deterministic jitter to avoid replica-wide retry herds. Not slower because `delivered` and the queryable `control` event are how an operator confirms a `stop` reached the trace.
- **Operational state is explicit.** gRPC health reports command acceptance, fanout, and the admission accelerator separately, so a broker or Valkey outage does not falsely make durable command acceptance look unavailable while a dependency is visibly degraded. The optional loopback Prometheus endpoint exposes the same low-cardinality counters and gauges. Malformed or repeatedly failing durable rows are quarantined after the bounded retry ceiling instead of hot-looping forever. SIGINT or SIGTERM triggers `NOT_SERVING`, wakes all workers, allows one bounded fanout drain, and awaits worker termination before process exit.
...
- **A shared outbox with a process-local inbox would have shipped a kill switch that works on one replica in N.** The Postgres outbox exists precisely so replicas can share durability; the inbox now follows it into a separate `apex_control_inbox` table, with row-locked claims and durable delivery attempts. The cross-replica proof accepts on replica A and polls on replica B, so sticky routing is no longer part of the control-channel correctness argument.
...
**0b. The delivery mechanism is polling only.** A JetStream per-agent subject or a long-poll remains open (see the unary-vs-streaming reasoning above), and [[Human-in-the-Loop Approvals]]'s blocking mode likely wants the same infrastructure -- whoever designs it should design it once for both. The delivery acknowledgement state is now shared when the gateway uses Postgres; the file backend remains intentionally single-writer for the local deployment shape.
...
3. **~~No health or metrics surface for "the accelerator is sidelined."~~ Closed by the operational hardening pass.** The gateway now samples `FallbackEphemeralStore::accelerator_sidelined()`, exposes the state through gRPC health and a loopback Prometheus endpoint, and retains the local ceiling as the authoritative fallback.
