...
def _command(action, *, command_id, reason_code=None):
...
def test_a_pending_action_this_pass_does_not_enact_is_inert():
    # `set_budget` is retrieved and deliberately not acted on yet. An agent
    # that silently ignores it is honest; one that pretends to enforce it is
    # not.
    sink = Sink()
    observer = BoundedObserver(sink)
    try:
        events = _loop(
            observer, control=InMemoryControlPoller([_command("set_budget", command_id="cmd-budget")])
